...
目前原始碼版本為 `0.4.7`（2026-09-15），ESP32 韌體版本仍為 `0.1.24+25`。單燈維持簡單的預設體驗並自動跟隨最新工作；連接多盞燈後，每盞燈可獨立跟隨最新工作、指定專案或 Claude Code/Codex/WorkBuddy/CodeBuddy/Trae 等指定 Agent，並可使用獨立燈光方案。
...
**權限閘門（安全提醒）**：Trae、WorkBuddy、CodeBuddy 這三個助手沒有原生的權限彈窗。在設定頁切到對應助手標籤，把「請求權限（permission-request）」事件行的顏色設為非「關閉」即開啟權限閘門（預設即為紅色 + 閃爍），此時**每一次工具呼叫都會請使用者確認並點亮紅燈**，與具體執行什麼命令無關，無需維護危險命令規則；設為「關閉」即停用閘門。
