...
La version actuelle du code source est `0.4.7` (2026-09-15) et le firmware ESP32 reste en `0.1.24+25`. Un seul voyant conserve le fonctionnement simple et suit la tâche la plus récente. Avec plusieurs voyants, chacun peut suivre indépendamment la dernière tâche, un projet ou un agent tel que Claude Code, Codex, WorkBuddy, CodeBuddy ou Trae, et utiliser son propre profil lumineux.
...
**Porte de sécurité (rappel) :** Les assistants Trae, WorkBuddy et CodeBuddy n'ont pas de fenêtre de permission native. Dans la page de configuration, passez à l'onglet de l'assistant concerné et réglez la couleur de la ligne d'événement **« Demande de permission » (permission-request)** sur une valeur autre que « Désactivé » pour activer la porte de sécurité (rouge + clignotement par défaut). Alors **chaque appel d'outil demandera confirmation à l'utilisateur et allumera la lumière rouge**, quel que soit la commande exécutée ; aucune liste de commandes dangereuses n'est à maintenir. Régler sur « Désactivé » désactive la porte.
