...
現在のソースバージョンは `0.4.1`（2026-08-14）、ESP32 ファームウェアは引き続き `0.1.21+22` です。ライトが 1 台の場合は、最新タスクに追従する従来のシンプルな動作を維持します。複数台を接続すると、各ライトを最新タスク、指定プロジェクト、Claude Code/Codex/WorkBuddy/CodeBuddy などの指定 Agent に個別に割り当て、独立したライト設定を使用できます。
