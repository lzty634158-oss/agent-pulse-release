...
現在のソースバージョンは `0.4.7`（2026-09-15）、ESP32 ファームウェアは引き続き `0.1.24+25` です。ライトが 1 台の場合は、最新タスクに追従する従来のシンプルな動作を維持します。複数台を接続すると、各ライトを最新タスク、指定プロジェクト、Claude Code/Codex/WorkBuddy/CodeBuddy/Trae などの指定 Agent に個別に割り当て、独立したライト設定を使用できます。

このリリースでは、接続中の BLE/USB デバイスを接続状態、バッテリー、充電状態とともに表示し続け、重複する BLE Bridge プロセスを防止します。フローティングウィンドウには最新タスクの Agent とプロジェクトも表示します。Bluetooth は近接自動接続と Windows のシステムペアリング接続を引き続き利用できます。更新は Gitee を優先し、失敗時に GitHub へ自動フォールバックします。詳細は [更新履歴](CHANGELOG.md) を参照してください。
...
**セキュリティゲート（注意）:** Trae、WorkBuddy、CodeBuddy の各アシスタントにはネイティブな権限ダイアログがありません。設定ページで該当するアシスタントのタブに切り替え、イベント行 **「権限リクエスト (permission-request)」** の色を「オフ」以外に設定するとセキュリティゲートが有効になります（既定は赤＋点滅）。有効時は**どのツール呼び出しでもユーザーに確認を求め、赤ランプが点灯**します。実行するコマンドに関係なく、危険コマンドのリストを管理する必要はありません。「オフ」にするとゲートが無効になります。
...
1. ライトの USB データケーブルを外して電源を入れます。アドバタイズが停止している場合は、ボタンを短押しして再開します。
2. Dashboard を開きます。未バインド時は、自動バインド、手動バインド、またはユーザーがスキャンを停止するまで継続的にスキャンします。
3. 対象ライトをこのコンピューターの Bluetooth アダプターへ近づけます。一覧には名前、MAC/デバイス識別子、RSSI、サンプル数がリアルタイム表示されます。
4. 自動バインドには 3 回以上のサンプル、`-45 dBm` 以上の RSSI、2 番目に強い候補より `8 dB` 以上強いことが必要です。受信強度の差で条件を満たさない場合は、対象行の「バインド」を選択します。

バインド後はスキャンを停止し、識別子をローカル設定に保存します。以後はそのライトだけへ自動接続し、信号強度で別のデバイスへ切り替えません。交換する場合は「デバイスを忘れる」を選び、近接または手動バインドをやり直します。

BLE アイコンは、青がスキャン/接続中、緑が最近の有効な通信、灰が未接続、赤がエラーです。接続後は現在の有効な状態だけを同期し、期限切れの過去のライトイベントは再送しません。Windows では通常 Bluetooth MAC が表示されますが、macOS では CoreBluetooth のプライバシー仕様により OS の UUID が表示される場合があります。この UUID はローカルバインドに使用できますが、ハードウェア MAC ではありません。

見つからない場合は、電源とアドバタイズ、システム Bluetooth、USB が未接続であること、別の Agent Pulse/BLE Bridge が動作していないことを確認してください。macOS では初回に Bluetooth 権限の許可も必要です。

#### USB シリアル接続、選択、復旧

1. データ通信対応の USB ケーブルを使用します。充電専用ケーブルではシリアルポートが作成されません。
2. Dashboard の「USB シリアル」パネルを開きます。Windows は `COM*`、macOS は通常 `/dev/cu.usbmodem*` または `/dev/cu.usbserial*` を表示します。
3. 既定の自動選択は AgentPulse のドライバーレスデバイス `VID:PID 303A:1001` だけに接続し、CH340、CP210x、FTDI などは自動的に開きません。
4. 複数の `303A:1001` デバイスがある場合や別の互換アダプターを使う場合は、一覧のポートと VID/PID を確認して手動選択します。

手動選択はローカル設定に保存されます。指定ポートがないときはオフラインを維持し、別のポートへ勝手に切り替えません。自動選択へ戻すこともできます。一覧には既定、選択済み、接続済み、未接続が表示され、ファームウェア対応時はヘッダーにバッテリーと充電状態も表示されます。

USB は BLE より優先されます。USB 接続中は BLE のスキャンと接続を停止し、USB 切断後にスキャンまたはバインド済みデバイスへの再接続を再開します。ポートが表示されない場合は、ケーブル、ドライバー、USB CDC ファームウェアを確認してください。USB は初回書き込み、パーティション移行、OTA 失敗後の優先復旧手段です。
...
- 複数デバイス環境では一意のデバイス識別子を永続バインドします。RSSI は初回の近接選択にだけ使用され、バインド済みライトを切り替えません。Windows では通常 MAC、macOS では CoreBluetooth UUID が使われる場合があります。
