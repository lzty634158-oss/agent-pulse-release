...
현재 소스 버전은 `0.4.7`(2026-09-15)이며 ESP32 펌웨어는 계속 `0.1.24+25`입니다. 표시등 한 개는 최신 작업을 따르는 기존의 간단한 동작을 유지합니다. 여러 표시등을 연결하면 각 표시등이 최신 작업, 특정 프로젝트 또는 Claude Code/Codex/WorkBuddy/CodeBuddy/Trae 같은 특정 Agent를 독립적으로 따르고 별도의 조명 프로필을 사용할 수 있습니다.
...
**보안 게이트(참고):** Trae, WorkBuddy, CodeBuddy 같은 도우미는 네이티브 권한 대화상자가 없습니다. 구성 페이지에서 해당 도우미 탭으로 전환한 다음 이벤트 행 **"권한 요청(permission-request)"** 의 색을 "끄기" 이외로 설정하면 보안 게이트가 켜집니다(기본값은 빨강 + 점멸). 활성화되면 **어떤 도구 호출이든 사용자에게 확인을 요청하고 빨간 표시등이 켜지며**, 실행하는 명령과 무관하게 위험 명령 목록을 관리할 필요가 없습니다. "끄기"로 설정하면 게이트가 비활성화됩니다.
